...
/// [_leb128Value]) — used to synthesize wire-shaped fixture replies.
Uint8List _leb128Encode(int value) {
  final out = <int>[];
  var v = value;
  while (true) {
    final b = v & 0x7f;
...
void main() {
  ...
  test('fetchObjects sends more than 100 uuids as a single request, no chunking', () async {
    ...
    serverSub = fakeServer.listen((client) {
      ...
      client.listen((chunk) {
        ...
        while (bytes.length >= 8) {
          ...
          lastBHeader = requestBody.sublist(bStart, bStart + 4);
    ...
    // The B section's second byte is the count varint's own LEB128 byte
    // width -- `01 02 96 01` for 150 uuids (real 128-uuid captures read
    // `01 02 80 01`; see _sendBatchGetObject). A hardcoded `01 01` here
    // was only byte-correct below 128 uuids.
    expect(lastBHeader, [0x01, 0x02, 0x96, 0x01]);
  });
  ...
  test('addOrUpdateRoute generates a proper UUIDv4 (version/variant bits set), not fully random bytes', () async {
    // Bug found live 2026-08-14: a real plotter crashed/rebooted on a GPX
    // route import (remote_helm_re/pcaps/2026-08-14_routeupload_gpx-import
    // -vlissingen-crash.pcap). Every uuid in every real capture on file is
    // a proper UUIDv4 (version nibble 4, variant bits 10) -- this client's
    // own generated uuid never was, until this fix. Extracts the uuid
    // straight from the raw uuid parameter passed to addOrUpdateRoute
    // (there's no server round-trip needed to see the generated bytes:
    // the wire format embeds the uuid directly in the tDeleteEntry-shaped
    // create/update message this sends).
    final pushBytes = _hexToBytes(
      '1d0000000200ca01933069d66c050000d04dd4326d050000b8010107b40101b101050711101aa8'
      '8250c42847bbbdc33cab45cd61c20d8994b0fc2d1102190127900102'
      '01000f8a0188011f8b08000000000004034d8c3b0ec3201005739'
    ...
    await conn.pushes.first.timeout(const Duration(seconds: 2));
    final resultUuid = await conn.addOrUpdateRoute('CLAUDE_TEST', const [(55.0, 10.0)]);
    await Future<void>.delayed(const Duration(milliseconds: 50));

    expect(sentRest, isNotNull, reason: 'addOrUpdateRoute must have sent a message');
    final parts = resultUuid.split('-');
    expect(parts[2][0], '4', reason: 'uuid version nibble must be 4 (UUIDv4), matching every real capture on file');
    final variantNibble = int.parse(parts[3][0], radix: 16);
    expect(variantNibble & 0xc, 0x8, reason: 'uuid variant bits must be 10, matching every real capture on file');
  });

  test('a mid-sync tGetObject for a uuid this connection just wrote gets real content, not tGetObjectError', () async {
    // Bug found live 2026-08-14 (remote_helm_re/findings/00_STATUS.md
    // Update 68): a real plotter crashed/rebooted when its own mid-sync
    // tGetObject asked about a uuid this client had JUST written via
    // addOrUpdateRoute -- _autoReplyToServerGetObject used to blindly
    // answer any uuid-carrying request with tGetObjectError, which is fine
    // for a uuid this client genuinely has no content for, but apparently
    // not for one it just claimed to write. This simulates exactly that
    // inside a real fetchCatalog() call (the only place
    // _autoReplyToServerGetObject actually listens): create a route first
    // (with a known/fixed uuid, no server round-trip needed), then run a
    // sync where the fake server's tCatalogSyncReply is preceded by a
    // uuid-carrying tGetObject for that same uuid -- exactly the
    // "plotter asks about something mid-sync" shape a real capture showed.
    const knownUuid = '11111111-2222-4333-8444-555555555555';
    Uint8List? capturedReplyToMidSyncRequest;
    late StreamSubscription<Socket> serverSub;
    final fakeServer = await ServerSocket.bind(InternetAddress.loopbackIPv4, 0);
    ...
    serverSub = fakeServer.listen((client) {
      ...
      client.listen((chunk) {
        ...
        while (bytes.length >= 8) {
          ...
          if (msgType == tCatalogSync) {
            // _autoReplyToServerGetObject only starts listening once
            // _syncCatalog sends its own tCatalogSync (see that function's
            // own call site) -- so the mid-sync tGetObject must be sent
            // AFTER this, not right after registration, or it arrives
            // before anything is listening for it (matches the real
            // capture's own ordering: registration reply, tCatalogSync
            // request, THEN the plotter's own mid-sync tGetObject).
            if (topicId == topicRoutes) {
              final uuidBytes = _hexToBytes(knownUuid.replaceAll('-', ''));
              final midSyncBody = <int>[
                ...List<int>.filled(8, 0xaa), // arbitrary "version" the reply must echo back
                0x17, 0x01, 0x01, 0x01, 0x07, 0x11, 0x10,
                ...uuidBytes,
              ];
              final topicBytes = Uint8List(4)..buffer.asByteData().setUint32(0, topicRoutes, Endian.little);
              final inner = <int>[
                ...topicBytes,
                tGetObject & 0xff, (tGetObject >> 8) & 0xff,
                ...midSyncBody,
              ];
              client.add(_wrapMsgFrame(Uint8List.fromList(inner)));
            }
            // Minimal, valid-enough tCatalogSyncReply (0 entries) so
            // fetchCatalog completes -- content doesn't matter for this
            // test, only that the connection stays alive long enough for
            // the mid-sync tGetObject's reply to be observed. Same shape
            // the existing differential-sync test above uses.
            final requestBody = request.sublist(6);
          ...
          if (msgType == tGetObjectReply || msgType == tGetObjectError) {
            capturedReplyToMidSyncRequest = Uint8List.sublistView(request, 6);
          }
        }
        buf.clear();
        buf.add(bytes);
    ...
    await conn.addOrUpdateRoute('CLAUDE_TEST', const [(55.0, 10.0)], uuid: knownUuid);
    await conn.fetchCatalog(topicRoutes, timeout: const Duration(seconds: 2));
    await Future<void>.delayed(const Duration(milliseconds: 100));

    expect(capturedReplyToMidSyncRequest, isNotNull, reason: 'the mid-sync tGetObject must have gotten SOME reply');
    // The reply body starts with its own outer LEB128 length prefix (see
    // _buildGetObjectReplyBody), THEN the version -- skip that prefix
    // before checking the version matches what the (fake) server's
    // mid-sync request supplied (0xaa x8), not some other value.
    final prefixLen = _leb128Length(capturedReplyToMidSyncRequest!, 0);
    expect(
      Uint8List.sublistView(capturedReplyToMidSyncRequest!, prefixLen, prefixLen + 8),
      Uint8List.fromList(List<int>.filled(8, 0xaa)),
      reason: 'reply must echo the version from the mid-sync request',
    );
  });
  ...
  test('fetchCatalogAndObjects sends a byte-exact non-empty (differential) tCatalogSync when knownObjects is given', () async {
    // The first 5 records of the official app's own real non-empty
    // tCatalogSync request for topicRoutes (75 records total) from a real
    // capture (`0d2f840f`, a plotter user-data reset followed by a real
    // ActiveCaptain backup restore) -- see route_catalog.dart's
    // _buildCatalogSyncBody doc comment for the full wire-format
    // derivation. The uuid → vstamp associations below are the corrected
    // 2026-08-14 reading (vstamp trailer AFTER its uuid, exactly as a
    // tCatalogSyncReply record): an earlier version of this test carried
    // the same 5 uuids with every vstamp shifted to its neighboring
    // record, matching the builder bug this test now guards against.
    final knownObjects = [
      const DownloadedObject(uuid: '295c1769-77eb-4ed7-84a9-f1f27a01f194', name: '', points: [], vstamp: 10067535587),
      const DownloadedObject(uuid: 'd394f591-c50d-4f13-a497-900940c3b239', name: '', points: [], vstamp: 9978656180),
      const DownloadedObject(uuid: '28103258-ac2a-4993-821f-4eb63755e49f', name: '', points: [], vstamp: 12310396025),
      const DownloadedObject(uuid: '93ad0de3-2681-403c-8bef-42ef18c7c027', name: '', points: [], vstamp: 10420970862),
      const DownloadedObject(uuid: '637905a5-24e2-41e7-9a7d-2ca7fa9c0e03', name: '', points: [], vstamp: 12772976429),
    ];
    // Those 5 records' raw wire bytes, verbatim from the captured request
    // (each: 02 07 11 10 marker, 16-byte uuid, length byte 0x0d, 5-byte
    // LEB128 vstamp).
    const capturedRecordsHex =
        '02071110295c176977eb4ed784a9f1f27a01f1940de3cdc9c025'
        '02071110d394f591c50d4f13a497900940c3b2390db4eb989625'
        '0207111028103258ac2a4993821f4eb63755e49f0df9f886ee2d'
        '0207111093ad0de32681403c8bef42ef18c7c0270deeca8de926'
        '02071110637905a524e241e79a7d2ca7fa9c0e030dadced0ca2f';
    // No persistence-path isolation is needed anymore: as of 2026-08-15 the
    // registration stamp is generated fresh per connection (see
    // route_catalog.dart's _registerTopic) rather than persisted to disk, so
    // there is no shared on-disk file a previous run could poison. This test
    // asserts the differential tCatalogSync's byte format and that it reuses
    // its own connection's registration correlation id, neither of which
    // depends on cross-run stamp stability.
    late StreamSubscription<Socket> serverSub;
    final fakeServer = await ServerSocket.bind(InternetAddress.loopbackIPv4, 0);
    ...
    serverSub = fakeServer.listen((client) {
      ...
      client.listen((chunk) {
        ...
        while (bytes.length >= 8) {
          ...
          if (msgType == tGetObject) {
            // The differential flow follows its sync with one version-delta
            // tGetObject -- echo it back as a nothing-new tGetObjectReply,
            // the same 19-byte echo a real plotter sends (see
            // fetchNewObjects' doc comment).
            final replyInner = Uint8List(6 + request.length - 6);
            ByteData.view(replyInner.buffer).setUint32(0, topicId, Endian.little);
            ByteData.view(replyInner.buffer).setUint16(4, tGetObjectReply, Endian.little);
            replyInner.setRange(6, replyInner.length, request.sublist(6));
            client.add(_wrapMsgFrame(replyInner));
          }
        }
        buf.clear();
    ...
    await conn.fetchCatalogAndObjects(
      topicRoutes,
      knownObjects: knownObjects,
      catalogTimeout: const Duration(seconds: 2),
      objectsTimeout: const Duration(seconds: 2),
    );

    expect(receivedSyncRequest, isNotNull);
    final sentBody = receivedSyncRequest!.sublist(6);

    // **The correlation-id bug this test guards against, found live
    // 2026-08-13**: the non-empty sync's correlation id must be
    // topicRoutes' own tRegisterTopic correlation id, not a
    // freshly-generated one -- two independent real captures showed this
    // reuse, and picking a fresh value instead got no reply from a real
    // plotter, twice.
    ...
    // Byte-exact expected body, per the real capture's structure (see
    // _buildCatalogSyncBody's doc comment): LEB128 outerLen 145 (`91 01`
    // -- 8-byte correlation id + 2-byte innerLen field + 135-byte inner),
    // the registration correlation id, LEB128 innerLen 135 (`87 01`), the
    // `02 01 05 09 00` header (count 5, one LEB128 byte wide, extraCount
    // 0), then the 5 real captured records verbatim.
    final expected = <int>[
      0x91, 0x01,
      ...registrationCorrelationId!,
      0x87, 0x01,
      0x02, 0x01, 0x05, 0x09, 0x00,
      ..._hexToBytes(capturedRecordsHex),
    ];
    expect(
      sentBody,
      expected,
      reason: 'non-empty tCatalogSync must match the real captured digest layout byte-for-byte',
    );
  });

  test('fetchCatalog encodes the vstamp length byte as actualLebLength+8, not always 0x0d', () async {
    // The vstamp trailer's length byte is the vstamp's actual LEB128 byte
    // count plus 8 -- confirmed against all 204 records of `0d2f840f`'s
    // three real digests (every 5-byte LEB128 vstamp carries 0x0d, the one
    // 6-byte vstamp carries 0x0e; a 1-byte vstamp gives 1+8=0x09 by the
    // same rule). This was previously hardcoded to always emit 0x0d
    // regardless of the real vstamp's LEB128 length, which is only
    // byte-correct for the common (large vstamp) case.
    final knownObjects = [
      const DownloadedObject(uuid: 'd394f591-c50d-4f13-a497-900940c3b239', name: '', points: [], vstamp: 1),
      const DownloadedObject(uuid: '28103258-ac2a-4993-821f-4eb63755e49f', name: '', points: [], vstamp: 10067535587),
    ];
    ...
    serverSub = fakeServer.listen((client) {
      ...
      client.listen((chunk) {
        ...
        while (bytes.length >= 8) {
          ...
          if (msgType == tGetObject) {
            // Echo the differential flow's version-delta tGetObject back
            // as a nothing-new reply -- see fetchNewObjects' doc comment.
            final replyInner = Uint8List(6 + request.length - 6);
            ByteData.view(replyInner.buffer).setUint32(0, topicId, Endian.little);
            ByteData.view(replyInner.buffer).setUint16(4, tGetObjectReply, Endian.little);
            replyInner.setRange(6, replyInner.length, request.sublist(6));
    ...
    // Each record is [02 07 11 10 marker][16-byte uuid][lengthByte]
    // [leb128 vstamp] -- the vstamp trailer FOLLOWS its uuid (see
    // _buildCatalogSyncBody's doc comment for the corrected 2026-08-14
    // record alignment), so the length byte under test sits right after
    // each entry's 16 uuid bytes.
    ...
    expect(
      sentBody.sublist(firstUuidIdx + 16, firstUuidIdx + 18),
      [0x09, 0x01],
      reason: 'vstamp=1 (1-byte leb128) should produce length byte 1+8=0x09 followed by the leb128 payload 0x01',
    );
    ...
    expect(
      sentBody.sublist(secondUuidIdx + 16, secondUuidIdx + 22),
      [0x0d, 0xe3, 0xcd, 0xc9, 0xc0, 0x25],
      reason: 'vstamp=10067535587 (5-byte leb128) should produce length byte 5+8=0x0d followed by its leb128 payload',
    );
  });

  test('a differential sync sends ONE version-delta tGetObject (byte-exact) and merges the delta over knownObjects', () async {
    // The full differential flow against real captured bytes: the sync
    // reply is the real 101-record tCatalogSyncReply fixture (whose
    // remote_ver is 984dd4320f030000), so the follow-up content request
    // must be the real app's single 13-byte-body version-delta tGetObject
    // -- `0c <remote_ver> 03 01 01 00`, an empty-uuid-list batch request
    // (see fetchNewObjects' doc comment; the same 19-byte-on-the-wire
    // shape appears in every differential real capture on file:
    // `0d2f840f`, `bb58f5ed`, `4fdcc705`). The fake server answers it with
    // the real captured tGetObjectReply for the "YYYY" route (whose own
    // correlation-id field already IS this remote_ver, straight from the
    // same real session -- no patching), so the merge must:
    //  - take the freshly-downloaded "YYYY" object over the stale cached
    //    copy of the same uuid,
    //  - keep the untouched cached object whose uuid the catalog still
    //    lists,
    //  - drop the cached object whose uuid the catalog no longer lists.
    final knownObjects = [
      // Stale copy of the object the delta reply carries fresh content for.
      const DownloadedObject(uuid: _routeUuid, name: 'stale-name', points: [], vstamp: 1),
      // Still listed in the catalog reply (its very first record) -- must
      // survive the merge untouched.
      const DownloadedObject(
        uuid: '295c1769-77eb-4ed7-84a9-f1f27a01f194',
        name: 'cached-route',
        points: [RoutePoint(name: 'cached-route', lat: 55.0, lon: 10.0)],
        vstamp: 10067535587,
      ),
      // Not listed anywhere in the catalog reply -- must drop out (that's
      // how a deletion made while this client was away propagates).
      const DownloadedObject(uuid: '00000000-0000-4000-8000-000000000001', name: 'deleted-meanwhile', points: [], vstamp: 2),
    ];
    ...
    serverSub = fakeServer.listen((client) {
      ...
      client.listen((chunk) {
        ...
        while (bytes.length >= 8) {
          ...
          if (msgType == tCatalogSync) {
            // The real captured reply, with only its echoed correlation id
            // patched to this request's own (the fixture reply's sits at
            // rest offset prefixLen+1, see _awaitCatalogSyncReply).
            final requestBody = request.sublist(6);
            final requestPrefixLen = _leb128Length(requestBody, 0);
            final correlationId = requestBody.sublist(requestPrefixLen, requestPrefixLen + 8);
            final reply = _hexToBytes(_capturedCatalogSyncReply);
            final replyRestPrefixLen = _leb128Length(reply, 6);
            reply.setRange(6 + replyRestPrefixLen + 1, 6 + replyRestPrefixLen + 1 + 8, correlationId);
            client.add(_wrapMsgFrame(reply));
            continue;
          }
          if (msgType == tGetObject) {
            getObjectRequests.add(request);
            // The real captured reply for the "YYYY" route -- its own
            // correlation-id field is already the fixture sync reply's
            // remote_ver (both fixtures come from the same real session).
            client.add(_wrapMsgFrame(_hexToBytes(_capturedGetObjectReply)));
          }
        }
        buf.clear();
        buf.add(bytes);
    ...
    final result = await conn.fetchCatalogAndObjects(
      topicRoutes,
      knownObjects: knownObjects,
      catalogTimeout: const Duration(seconds: 2),
      objectsTimeout: const Duration(seconds: 2),
    ...
    // Exactly ONE content request, byte-exact the real version-delta
    // shape: `0c` (LEB128 length 12) + the sync reply's remote_ver +
    // `03 01 01 00` (LEB128 length 3, then an empty uuid list: tag 01,
    // count width 01, count 00).
    expect(getObjectRequests, hasLength(1), reason: 'differential flow must send exactly one tGetObject, never a uuid-batch burst');
    expect(getObjectRequests.single.sublist(6), _hexToBytes('0c984dd4320f03000003010100'));

    final byUuid = {for (final o in result) o.uuid: o};
    expect(byUuid, hasLength(2));
    // Fresh delta content replaced the stale cached copy.
    expect(byUuid[_routeUuid]?.name, 'YYYY');
    expect(byUuid[_routeUuid]?.points, hasLength(4));
    expect(byUuid[_routeUuid]?.vstamp, 32662919676);
    // Still-listed cached object survived untouched.
    expect(byUuid['295c1769-77eb-4ed7-84a9-f1f27a01f194']?.name, 'cached-route');
    // No-longer-listed cached object dropped out.
    expect(byUuid.containsKey('00000000-0000-4000-8000-000000000001'), isFalse);
  });

  test('a first-ever sync (no knownObjects) downloads content with exactly ONE unchunked batch tGetObject', () async {
    // Real first-ever-sync captures (`9a59f873`, `355eb949`: a fresh app
    // against a plotter holding 63 routes/128 waypoints) show N=0 sync
    // followed by exactly ONE batch tGetObject listing every valid entry
    // -- never the chunked multi-request burst this client used to send,
    // which is the one behavior present in every capture of this client
    // crashing the plotter and absent from every good capture (see
    // fetchCatalogAndObjects' doc comment).
    late StreamSubscription<Socket> serverSub;
    final fakeServer = await ServerSocket.bind(InternetAddress.loopbackIPv4, 0);
    final getObjectRequests = <Uint8List>[];
    Uint8List? syncRequestBody;
    ...
    serverSub = fakeServer.listen((client) {
      ...
      client.listen((chunk) {
        ...
        while (bytes.length >= 8) {
          ...
          if (msgType == tCatalogSync) {
            syncRequestBody = request.sublist(6);
            // N=0 request: correlation id at a fixed offset 1 (1-byte
            // length field), per the real capture's 15-byte shape.
            final correlationId = syncRequestBody!.sublist(1, 9);
            final reply = _hexToBytes(_capturedCatalogSyncReply);
            final replyRestPrefixLen = _leb128Length(reply, 6);
            reply.setRange(6 + replyRestPrefixLen + 1, 6 + replyRestPrefixLen + 1 + 8, correlationId);
            client.add(_wrapMsgFrame(reply));
          ...
          if (msgType == tGetObject) {
            getObjectRequests.add(request);
            // Echo back as the reply -- no gzip content needed; this test
            // only checks the request side.
            final replyInner = Uint8List(request.length);
            replyInner.setRange(0, request.length, request);
            ByteData.view(replyInner.buffer).setUint16(4, tGetObjectReply, Endian.little);
            client.add(_wrapMsgFrame(replyInner));
          }
        }
    ...
    // The sync request itself must be the N=0 shape (15-byte body, fixed
    // tail) -- no knownObjects means no digest to send.
    expect(syncRequestBody, isNotNull);
    expect(syncRequestBody!.length, 15);
    expect(syncRequestBody!.sublist(9), [0x05, 0x02, 0x01, 0x00, 0x09, 0x00]);

    // Exactly ONE batch tGetObject, listing every valid catalog entry (the
    // fixture reply's own valid-count field says 74) with the correct
    // count-width byte.
    expect(getObjectRequests, hasLength(1), reason: 'first-ever sync must download in one batch, not a chunked burst');
    final requestBody = getObjectRequests.single.sublist(6);
    final prefixLen = _leb128Length(requestBody, 0);
    // The 8-byte field must be the fixture sync reply's own remote_ver.
    expect(requestBody.sublist(prefixLen, prefixLen + 8), _hexToBytes('984dd4320f030000'));
    final bLenOffset = prefixLen + 8;
    final bStart = bLenOffset + _leb128Length(requestBody, bLenOffset);
    // B section: tag 01, count width 01, LEB128 count 74 (0x4a).
    expect(requestBody.sublist(bStart, bStart + 3), [0x01, 0x01, 0x4a]);
    // 74 uuid records, each 01 07 11 10 + 16 uuid bytes.
    var markerCount = 0;
    var searchFrom = bStart + 3;
    while (true) {
      final idx = _findHex(Uint8List.sublistView(requestBody, searchFrom), '01071110');
      if (idx < 0) break;
      markerCount++;
      searchFrom += idx + 4 + 16;
    }
    expect(markerCount, 74);
  });

  test('fetchCatalog decodes a 2-LEB128-byte-wide valid-count header (128+ valid entries)', () async {
    // The byte after the header's leading 0x02 is the count varint's own
    // byte width, not a constant 0x01 -- the real 128-waypoint
    // tCatalogSyncReply reads `02 02 80 01 09 00` (count 128 needs two
    // LEB128 bytes). Every fixture used elsewhere in this file happens to
    // stay below 128 valid entries, so this synthesizes a 130-record reply
    // using that real header shape (count=128, extra=2) and expects the
    // valid-count trim to keep exactly the first 128.
    late StreamSubscription<Socket> serverSub;
    final fakeServer = await ServerSocket.bind(InternetAddress.loopbackIPv4, 0);

    Uint8List buildReply(Uint8List correlationId) {
      final records = BytesBuilder(copy: false);
      for (var i = 0; i < 130; i++) {
        records.add(const [0x02, 0x07, 0x11, 0x10]);
        final uuid = Uint8List(16);
        ByteData.view(uuid.buffer).setUint32(12, i);
        records.add(uuid);
        // 5-byte LEB128 vstamp trailer (0x0d = 5+8), keeping every record
        // the standard 26 bytes.
        records.add(const [0x0d, 0xe3, 0xcd, 0xc9, 0xc0, 0x25]);
      }
      final recordBytes = records.toBytes();
      const header = [0x02, 0x02, 0x80, 0x01, 0x09, 0x02]; // count=128 (2-byte leb128), extra=2
      final recordsSection = BytesBuilder(copy: false)
        ..add(header)
        ..add(recordBytes);
      final recordsSectionBytes = recordsSection.toBytes();

      final afterPrefix = BytesBuilder(copy: false)
        ..add(const [0x02])
        ..add(correlationId)
        ..add(Uint8List(8)) // field1 (unused by the parser)
        ..add(_fakeRemoteVer)
        ..add(_leb128Encode(recordsSectionBytes.length))
        ..add(recordsSectionBytes);
      final afterPrefixBytes = afterPrefix.toBytes();

      final rest = BytesBuilder(copy: false)
        ..add(_leb128Encode(afterPrefixBytes.length))
        ..add(afterPrefixBytes);
      final restBytes = rest.toBytes();

      final replyInner = Uint8List(6 + restBytes.length);
      ByteData.view(replyInner.buffer).setUint32(0, topicRoutes, Endian.little);
      ByteData.view(replyInner.buffer).setUint16(4, tCatalogSyncReply, Endian.little);
      replyInner.setRange(6, replyInner.length, restBytes);
      return replyInner;
    }
    ...
    serverSub = fakeServer.listen((client) {
      ...
      client.listen((chunk) {
        ...
        while (bytes.length >= 8) {
          ...
          if (msgType == tCatalogSync) {
            final requestBody = request.sublist(6);
            final correlationId = requestBody.sublist(1, 9); // N=0 shape
            client.add(_wrapMsgFrame(buildReply(correlationId)));
          }
        }
        buf.clear();
        buf.add(bytes);
    ...
    final entries = await conn.fetchCatalog(topicRoutes, timeout: const Duration(seconds: 2));
    expect(entries, hasLength(128), reason: 'valid-count 128 (2-byte LEB128) must be decoded and applied as the trim');
  });
}
